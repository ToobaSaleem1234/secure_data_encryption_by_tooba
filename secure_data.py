import streamlit as st       # make UI
import hashlib               #  convert passkey to hash 
import json                  # save our data in json format
import time                  # manage failed attempts
from cryptography.fernet import Fernet
import base64                # secure our data

# Initialize sessions state variables if they don't exist:
...
# Functions to hash passkey:
def hash_passkey(passkey):
    return hashlib.sha256(passkey.encode()).hexdigest()

# Functions to generate a key from passkey(for encryption)
def generate_key_from_passkey(passkey):
    # Use the passkey to create a consistent key:
    hashed = hashlib.sha256(passkey.encode()).digest()
    # Ensure its a valid for Fernet(32 url-safe base64-encode bytes)
...
def decrypt_data(encrypted_text, passkey, data_id):
    try:
        ...
        if data_id in st.session_state.stored_data and st.session_state.stored_data[data_id]['hashed_passkey'] == hashed_passkey:
            ...
            # Increement failed attempts:
            st.session_state.failed_attempts += 1
            st.session_state.last_attempt_time = time.time()
            st.warning("Invalid passkey or data ID.")
            return None
    except Exception as e:
        # If decryption fails, increement failed attempts
        st.session_state.failed_attempts += 1
        st.session_state.last_attempt_time = time.time()
        return None
...
# Login Page:
    ...
    if time.time() - st.session_state.last_attempt_time < 11 and st.session_state.failed_attempts >= 3:
        remaining_time = int(11 - (time.time()- st.session_state.last_attempt_time))
    ...
    if st.button("Login"):
        if login_pass == 'admin123':
            ...
            st.error("❌ Incorrect Master Passkey!")

# Add a footer:
st.markdown("---")
st.markdown("#### 🔒 Secure Data Encryption System | Educational Project by Tooba")